...
- `IGameDataFactory` (`DependencyInjection/`) — bundles `IDriversLoader`/`ITeamsLoader`/`ISeasonLoader`/`IGameStorage`/`IAccoladesLoader`; implemented by `Ams2StorageFactory` in the AMS2 project. This bundle only exposes the non-generic loader interfaces (`ISeasonLoader`, not `ISeasonLoader<Ams2Season>`) — `ISeasonLoader.LoadBaseSeason(year)`/`IDriversLoader.LoadDriversBase(year)` return `ISeason`/`Dictionary<string, IDriverData>` (the AMS2-generic-typed `LoadSeason`/`LoadDrivers` overloads return the concrete `Ams2Season`/`Ams2DriverData` shapes and only exist on `Ams2StorageFactory` itself, not on this interface). `MainWindow` depends on `IGameDataFactory` (not the concrete `Ams2StorageFactory`) for exactly this reason, using the `Base`-suffixed methods throughout.
- `IExternalLiveriesInstaller` (`Updater/`) — game-agnostic wrapper around downloading/extracting a season's external livery pack (`HasExternalLiveries(int)`/`InstallAsync(int, IExternalLiveriesPrompt)`), implemented by `ExternalLiveriesInstaller` in the AMS2 project (it's tied to AMS2's season-pack file format/config model, so the implementation stays AMS2-concrete even though the interface doesn't need to be). Mirrors `IExternalLiveriesPrompt`'s existing seam; `MainWindow`, `InstallSeasonModCommandAsync`, and `WpfSeasonDownloadPrompt` all depend on the interface, not the concrete class.
...
- `IPlayerCosmeticsEditor` — optional per-game cosmetics (e.g. helmet) editing. Resolved with `GetService` (nullable), not `GetRequiredService` — a game with no cosmetics concept simply doesn't register one; `MainWindow` hides the helmet-selection UI when it's null. Implemented by `Ams2PlayerCosmeticsEditor`, whose `ShowEditor(IPlayerData, ISaveGame, object ownerWindow)` internally opens `Ams2ChEd.Business.AMS2.UI.Ams2PlayerCosmeticsEditorWindow` — the WPF window itself lives in the AMS2 project, not in `AMS2ChEd`, so callers never reference it directly.
...
- `IGameInstallSettingsStorage` (`Settings/Contracts`) — game-agnostic `GameInstallFolder` data, plus two behavior hooks: `NeedsPlayerSetup()` (does the player still need to provide required per-game setup before racing?) and `ShowEditor(object ownerWindow)` (show the game-specific settings UI, `ownerWindow` passed through as an opaque handle exactly like `IPlayerCosmeticsEditor.ShowEditor`). Implemented by `Ams2GameInstallSettingsStorage`, which additionally tracks an AMS2-only in-game driver name (`LoadInGameName`/`SaveInGameName`, not part of the interface — this is deliberately AMS2-concrete-only, not generalized) used by `NeedsPlayerSetup`/`ShowEditor` and by `Ams2RaceDataService` (via a cast) to match shared-memory telemetry participants against the human player. Its editor UI, `Ams2ChEd.Business.AMS2.UI.OptionsWindow`, lives in the AMS2 project for the same reason as the cosmetics editor window above — `MainWindow`/`EntryListWindow` call `.ShowEditor(this)`/`.NeedsPlayerSetup()` on the interface and never reference the window or the AMS2-concrete storage class directly. This is newer and narrower than the older AMS2-specific `IAms2AppSettingsStorage`/`Ams2SettingsStorage`, which `PerformanceCalibrationDialog` (SeasonPackCreator only) still consumes directly for AMS2-only settings — both currently coexist.
