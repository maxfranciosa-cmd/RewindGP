...
- `IGameDataFactory` (`DependencyInjection/`) — bundles `IDriversLoader`/`ITeamsLoader`/`ISeasonLoader`/`IGameStorage`/`IAccoladesLoader`; implemented by `Ams2StorageFactory` in the AMS2 project. Note this bundle only exposes the non-generic loader interfaces (`ISeasonLoader`, not `ISeasonLoader<Ams2Season>`) — `MainWindow` still depends on the concrete `Ams2StorageFactory` type directly (a known, pre-existing violation of the "only App.xaml.cs touches Ams2ChEd.Business.AMS2 directly" rule stated in `AMS2ChEd.csproj`), because it calls `SeasonLoader.LoadSeason(year)`/`DriversLoader.LoadDrivers(year)`, which only exist on the AMS2-generic-typed loader interfaces, not on this bundle. Fully fixing that would need a larger change (e.g. a generic `IGameDataFactory<TDriverData, TSeason>`) — out of scope so far.
